...
- wezterm's Windows default shell is `default_prog = { "wsl.exe", "~" }`, not a WSL domain. `default_gui_startup_args = { "connect", "unix" }` opens the first window in the `unix` domain, so `default_domain` never applies, and a `wsl_domains` pane would be owned by the GUI process instead of the mux server, losing the session on restart. Running the mux server inside WSL is WSL 1 only (WSL 2 has no AF_UNIX interop). pwsh therefore lives in `launch_menu`, reached with `LEADER n`.
- A WSL pane's cwd reaches wezterm only through OSC 7: its process is the Windows-side `wsl.exe`/`wslhost.exe`, so process introspection reports the Windows cwd (`file:///C:/Users/...`), not the Linux one. `mise/dotfiles/.config/zsh/wezterm.zsh` publishes the raw Linux path, and because Windows cannot `chdir` there, `spawn_in_current_cwd` in `keybinds.lua` builds `wsl.exe --cd <path>` for splits and new tabs — the same argv wezterm's own `WslDomain` uses. The two must change together. Do not put a UNC path (`wslpath -m`) in OSC 7: wezterm discards the URL host and would try to `chdir` to `/wsl.localhost/<distro>/...`.
- Do not reach for OSC 133 semantic zones in wezterm: `codec` never carries a `SemanticZone` and `ClientPane` does not implement `get_semantic_zones`, so `pane:get_semantic_zones()` returns an empty list for every pane in the `unix` mux domain — which is all of them. `ScrollToPrompt` and CopyMode's `MoveBackwardZoneOfType` are dead for the same reason. `LEADER c` (`copy_last_output` in `keybinds.lua`) therefore locates the previous command's output by walking rows: it skips the current prompt block up to the blank line `add_newline` emits, then looks for the preceding line starting with `PROMPT_SYMBOL`. That symbol and the three-line prompt shape come from `mise/dotfiles/.config/starship.toml` (`format`, `add_newline`, `[character]`), which stays the single source of truth — change `keybinds.lua` with it.
