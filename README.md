...
mise -C mise -E darwin dotfiles status
mise -C mise -E linux dotfiles status
```
...
Microsoft Office、Obsidian、Spark、Spotify、WezTerm Nightly、Zen Twilight、Zoom を
...
`windows`、`windows-powershell` の順に処理します。前者だけがNeovim、Git、WezTermを、
後者だけが重複し得るcombined mise config、starship、PowerShell profileをapplyします。
MinGit install時だけArtifact Attestationsを無効化するhookを先に実行します。
